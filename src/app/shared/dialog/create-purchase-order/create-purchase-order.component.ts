...
export class CreatePurchaseOrderComponent implements OnInit {
  ...
  pobillNumbercheck(event, mode) {
    this.general.SearchBillPONO(this.userWorkshopid, event, mode).subscribe(
      (datacheck) => {
    ...
    this.general.PODetail(this.userWorkshopid, this.counternumber).subscribe(
      (coounterdata) => {
        ...
        if (coounterdata.success == true) { 
          ...
          this.showtext = true;
          this.CreateCustomerForm.controls["master_supplier_id"].setValue(coounterdata.order.master_supplier_id);
          ...
          if (coounterdata.order.balance_amount != "0") {
            if(this.receivedStage || this.closedStage){
              this.prebalance =
              parseFloat(this.cusbla) -
              parseFloat(coounterdata.order.balance_amount);
              this.backupPrebalance = parseFloat(this.cusbla) - parseFloat(coounterdata.order.balance_amount);
              // this.totalbalance = parseFloat(this.cusbla);
              this.totalbalance = this.prebalance + parseFloat(coounterdata.order.balance_amount);
              this.backuptotalbalnce = this.prebalance + parseFloat(coounterdata.order.balance_amount);
            }else{
              this.prebalance = parseFloat(this.cusbla);
              this.backupPrebalance = parseFloat(this.cusbla);
              this.totalbalance = parseFloat(this.cusbla) + parseFloat(coounterdata.order.balance_amount);
              this.backuptotalbalnce = parseFloat(this.cusbla) + parseFloat(coounterdata.order.balance_amount);
            } 
          } else {
            this.prebalance = 0;
            this.totalbalance =
          ...
          // this.jobtaxableamount = this.calcultetaxableamount(coounterdata.order) 
          this.jobtaxableamount = parseFloat(coounterdata.order.taxable) || 0
          ...
          this.recveedambkp = parseFloat(coounterdata.order.paid_amount);
  ...
  new_cal() {
    this.jobdatatabel.map((spareData) => {
      console.log("----------",spareData.amount / spareData.quantity)
  ...
  receviedtype(event) {
    ...
    if (event == "partial") {
      this.showre = false;
      this.recveedam = this.recveedambkp;
      this.prebalance = this.backupPrebalance;
      this.balanceshow = this.JobTotal - this.discountamount - this.recveedam;
  ...
  receviedamount(event) {
    ...
    if (event.match(/^\d*\.?\d*$/)) {
      var checkbalall;
      checkbalall = (this.JobTotal - this.discountamount) - parseInt(event);
      if (checkbalall <= 0) {
        ...
        this.balanceshow =  (this.JobTotal - this.discountamount) - this.recveedam;
        // this.totalbalance = parseInt(this.cusbla) + this.balanceshow - this.prebalance;
        // this.totalbalance = parseInt(this.cusbla) + this.balanceshow;
        this.totalbalance = this.prebalance + this.balanceshow;
    ...
    // if (event.match(/^\d*\.?\d*$/)) {
    //   var checkbalall;
    //   checkbalall = this.prebalance + this.JobTotal - this.discountamount - parseInt(event);
    //   console.log("this.prebalance",this.prebalance,this.JobTotal,this.discountamount,parseInt(event))
    //   console.log("checkbalall",checkbalall)
    //   if (checkbalall < 0) {
    //     this.showerrorinfield = true;
    //     this.showerrorre = true;
    //     this.recveedam = event;
    ...
    //       console.log("this.JobTotal",this.JobTotal - this.discountamount - this.recveedam)
    //       this.balanceshow = 0;
    //       console.log("this.backupbalance",this.backupbalance)
  ...
  jobdiscountvalue(event, indexdata, index) {
    if (event.match(/^\d*\.?\d*$/)) {
      ...
      this.jobdatatabel[index]["unit_purchase_price"] = parseInt(totalcal[0]["unit_price"]);
  ...
  checkvalueenteredforjob(event, index, indexdata, falg) {
    if (event.match(/^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)$/)) {
      ...
      this.jobdatatabel[index]["unit_purchase_price"] =
        parseFloat(totalcal[0]["unit_price"]);
  ...
  selectedResult(event) {
    if (this.allcustomuerdata.length != 0) {
      ...
      console.log("cusinfo",cusinfo, cusinfo[0].master_supplier_id);
      ...
      this.CreateCustomerForm.controls["mobileOneNo"].setValue(
        cusinfo[0].supplier_mobile1,
        { onlySelf: true }
      );
      this.CreateCustomerForm.controls["master_supplier_id"].setValue(
        cusinfo[0].master_supplier_id,
        { onlySelf: true }
      );
      this.showtext = true;
      this.showeditcus = true;
  ...
  addnewcustomer() {
    ...
    this.CreateCustomerForm.controls["master_supplier_id"].setValue("", { onlySelf: true });
        ...
        .subscribe(
          (cusinfo) => {
            ...
            if (cusinfo.success == true) {
              ...
              this.CreateSupplierForm.controls["firstname"].setValue(
                cusinfo["customer"]["supplier_name"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["contact1"].setValue(
                cusinfo["customer"]["supplier_mobile1"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["contact2"].setValue(
                cusinfo["customer"]["supplier_mobile2"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["email"].setValue(
                cusinfo["customer"]["supplier_email"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["firm"].setValue(
                cusinfo["customer"]["business_name"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["address"].setValue(
                cusinfo["customer"]["supplier_address"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["gst"].setValue(
                cusinfo["customer"]["supplier_gst_no"],
                { onlySelf: true }
              );
              this.CreateSupplierForm.controls["gst"].setValue(
                cusinfo["customer"]["master_supplier_id"],
                { onlySelf: true }
              );
            }
            this.showspinner.setSpinner(false);
  ...
  createSupplier(evnet) {
    if (evnet == "update") {
      this.general
        .CreateUpdateSupplier(
          "update",
          this.userWorkshopid,
          this.CreateSupplierForm.getRawValue().firstname,
          this.CreateSupplierForm.getRawValue().firm,
          this.CreateSupplierForm.getRawValue().contact1,
          this.CreateSupplierForm.getRawValue().contact2,
          this.CreateSupplierForm.getRawValue().email,
          this.CreateSupplierForm.getRawValue().address,
          this.CreateSupplierForm.getRawValue().gst,
          this.CreateSupplierForm.getRawValue().id,
          this.CreateSupplierForm.getRawValue().master_supplier_id
        )
        .subscribe(
          (dataupdate) => {
            this.showspinner.setSpinner(true);
      ...
      if(this.CreateSupplierForm.value.contact1 != this.userService.getData()["workshop_mobile_number_1"]){
        ...
        rD.password = '';
        rD.role = 'supplier';
        this.general.registerUser(rD.email, rD.first_name, rD.last_name, rD.mobile_number_1, 
          ...
          rD.country_currency, rD.latlong, rD.primary_language,rD.password,rD.role).subscribe(registerData=>{
            if(registerData.success || !registerData.success){
            var masterSupplierId = '0';
              if(registerData.role){
                masterSupplierId = registerData.workshop_id || this.CreateSupplierForm.getRawValue().master_supplier_id;
              }else{
                masterSupplierId = registerData.data[0].workshop_id || this.CreateSupplierForm.getRawValue().master_supplier_id;
              }
              this.general.CreateUpdateSupplier(
                  "create",
                  this.userWorkshopid,
                  this.CreateSupplierForm.getRawValue().firstname,
                  this.CreateSupplierForm.getRawValue().firm,
  ...
  cancelupdate() {
    if (this.counterOpenOrEdit != "edit") {
      ...
      this.CreateCustomerForm.controls["master_supplier_id"].setValue(this.masterSupplierId, {
        onlySelf: true,
      });
      this.CreateCustomerForm.controls["customername"].disable();
    ...
    this.general.SupplierDashboard(this.userWorkshopid, event).subscribe(
      (dashResult) => {
        if (dashResult.success == false) {
          ...
          this.cusname = dashResult["supplier"].supplier_name;
          this.custid = dashResult["supplier"].id;
          this.cusphone = dashResult["supplier"].supplier_mobile1;
          this.masterSupplierId = dashResult["supplier"].master_supplier_id;
  ...
  // close tyhe popup
  closePopup(e) {
    if(e == 'close' || this.cancelStage || this.closedStage){
      this.dialogRef.close("okay");
    }else{
      if(this.openStage || this.placedStage){
        this.UpdatePOStatus('cancel');
      }
      this.dialogRef.close("okay");
    }
  }
  ...
  createPO(mode) {
    ...
    // console.log("this.po_status",this.po_status);
    if (this.recveedam != 0) {
    }
    if(mode == 'create'){
      po_status = 0;
    }else if(mode =='PlaceOrder'){
      po_status = 1;
      mode = 'create';
    }else{
      po_status =  parseInt(this.po_status);
    }
    this.general.CreateUpdatePO(
        mode,
        this.userWorkshopid,
        this.counterId,
        ...
        this.TransportForm.getRawValue().ltno, po_status,
        this.CreateCustomerForm.getRawValue().master_supplier_id || 0,
        this.DispatchDateValue
      ).subscribe(
        (createCounter) => {
...
getPoStatus(po){
console.log("postatus>>",po)
  switch(po) { 
    case '0': { 
      this.openStage = true;
      this.po_status_invoice = "open";
       break; 
    } 
    case '1': { 
      this.placedStage = true;
      this.po_status_invoice = "placed";
       break; 
    } 
    case '2': { 
      this.receivedStage = true; 
      this.po_status_invoice = "received";
      break; 
    } 
    case '3': { 
      this.closedStage = true; 
      this.po_status_invoice = "closed";
      break; 
    } 
    case '4': { 
      this.cancelStage = true;
      this.po_status_invoice = "cancel";
...
UpdatePOStatus(mode){
        ...
        .subscribe(
          (createCounter) => {
            ...
            if (createCounter.success == true) {
              ...
              // status Add to stock and stock present 
              if (poStatus===3 &&
    ...
    generatePdfPO(mode){
      this.allBillingData = {};
      this.allBillingData.supplierDetails = {};
      this.allBillingData.BillNo = this.counterId;
      // this.allBillingData.jobdatatabel = this.jobdatatabel.filter(item => item.received == true);
      this.allBillingData.jobdatatabel = this.jobdatatabel.filter(item => item.received == true && item.available == true);
      this.allBillingData.supplierDetails.customerid = this.CreateCustomerForm.getRawValue().customerid;
      this.allBillingData.poNumberdata = this.poNumberdata;
      this.allBillingData.JobTotal = this.JobTotal;
      this.allBillingData.discountvalue = this.discountvalue;
      this.allBillingData.discounttpe = this.discounttpe;
      this.allBillingData.balanceshow = this.balanceshow.toString();
      this.allBillingData.recveedam = this.recveedam.toString()
      this.allBillingData.calTotalGstForJobamount = this.calTotalGstForJobamount.toString()
      this.allBillingData.receviedty = this.receviedty;
      this.allBillingData.discountamount = this.discountamount;
      this.allBillingData.receviedtymethod = this.receviedtymethod;
      this.allBillingData.poDateValue = this.poDateValue;
      this.allBillingData.returnamount = this.returnamount;
      this.allBillingData.counterDatevalue = this.counterDatevalue;
      this.allBillingData.supplierDetails.customername = this.CreateCustomerForm.getRawValue().customername;
      this.allBillingData.supplierDetails.mobileOneNo = this.CreateCustomerForm.getRawValue().mobileOneNo;
      this.allBillingData.supplierDetails.email = this.CreateCustomerForm.getRawValue().email,
      this.allBillingData.supplierDetails.address =this.CreateCustomerForm.getRawValue().address,
      this.allBillingData.supplierDetails.gstno =this.CreateCustomerForm.getRawValue().gstno,
      this.allBillingData.exitdata = this.exitdata;
      this.allBillingData.receviedDate = this.receviedDate;
      this.allBillingData.returndate = this.returndate;
      this.allBillingData.refundamount = this.refundamount;
      this.allBillingData.isredund = this.isredund;
      this.allBillingData.TransportForm = this.TransportForm;
      this.allBillingData.DeliveryDateValue = this.DeliveryDateValue;
      this.allBillingData.totalbalance = this.totalbalance;
      this.allBillingData.vehicle = this.TransportForm.getRawValue().vehicle;
      this.allBillingData.person = this.TransportForm.getRawValue().person;
      this.allBillingData.transport = this.TransportForm.getRawValue().transport;
      this.allBillingData.ltno = this.TransportForm.getRawValue().ltno;
      this.allBillingData.poStatus = this.po_status_invoice;
      this.allBillingData.po_status = this.po_status;
      this.allBillingData.counternumber = this.counternumber
      


      this.dialogservice
      .OpenInvoiceEstimateForPO("Purchase Order", "100%", "100%", this.allBillingData)
      .subscribe((data) => {});
    }
    
    notReceivedJobItem(indexdata, index){
      var event = 0;
        var amount =
          parseFloat(indexdata["unit_purchase_price"]) * event;
        this.jobdatatabel[index]["current_quantity"] =
        event + parseFloat(this.jobdatatabel[index]["alterqty"]);
