...
@Component({
  selector: "app-create-short-invoice",
  templateUrl: "./create-short-invoice.component.html",
  styleUrls: ["./create-short-invoice.component.css"],
  providers: [
    {
      provide: DateAdapter,
...
/**
 * In this file Short Invoice is generated
 ...
 * and then create it/
 */
export class CreateShortInvoiceComponent implements OnInit {
  sparemodel: any;
  ...
  // added this flag to disable the create button
  // to avoid mutiple request
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CreateShortInvoiceComponent>,
    ...
    this.minDate = new Date(Date.now() - 200 * 24 * 60 * 60 * 1000);
        ...
        this.general
          ...
          .subscribe((customerDetail) => {
            if (customerDetail.success == true) {
              ...
              //   console.log("this ran 333333333");
              this.existing_customer = customerDetail.data[0].customer_name;
              this.entered_number_valid = false;

              // console.log("existing_customer", this.existing_customer);
              // console.log("entered_number_valid", this.entered_number_valid);
              // }
      ...
      this.general
        ...
        .subscribe((customerDetail) => {
          if (customerDetail.success == true) {
            this.existing_customer = customerDetail.data[0].customer_name;
            this.entered_number_valid = false;
            // console.log("this.jobcardEditOrCreate ==  create");
          ...
          if (customerDetail.success == true) {
            ...
            if (this.jobcardEditOrCreate == "edit") {
              this.general
                ...
                .subscribe((vehicleIs) => {
                  // console.log("vehicle is", vehicleIs);
                  if (
                    ...
                    if (reminders.success == true) {
                      ...
                      if (
                        ...
                        reminders.reminders.map((data) => {
                          ...
                          if (data.message.includes("on today")) {
                        ...
                        reminders.reminders.map((datanew) => {
                          if (datanew.message.includes("on today")) {
                        ...
                        reminders.reminders.map((dataallnew) => {
                          if (dataallnew.message.includes("on today")) {
      ...
      (cusData) => {
        if (cusData.success == true) {
          ...
          for (var i = 0; i < cusData["data"].length; i++) {
            ...
            // console.log("vehicle_numbers_added", this.vehicle_numbers_added);
  ...
  addCustomerData(api_data) {
    if (api_data.success == true) {
      ...
      // console.log(" this.searchCustomerDataPage ", this.searchCustomerDataPage);
      // console.log(" this.searchCustomerData ", this.searchCustomerData);
  ...
  loadMoreCustomers() {
    // console.log("from loadMoreCustomers");
      ...
      .subscribe(
        (customerDetail) => {
          ...
          if (customerDetail.success == true) {
                ...
                this.general
                  ...
                  .subscribe((reminders) => {
                    ...
                    if (reminders.success == true) {
                      this.showspinner.setSpinnerForLogin(false);
                      reminders.reminders.map((data) => {
                        if (data.reminder_priority != "") {
                          if (
                            data.reminder_priority.toLowerCase() ==
                            "short_invoice"
                          ) {
                            if (data.message.includes("experience with us")) {
                              this.reminderOneDate = data.date;
                              this.reminderOnestatus = data.status;
                            }
                          }
            ...
            this.CreateCustomerForm.controls["mobileOneNo"].enable();
            this.CreateCustomerForm.controls["searchVehicle"].enable();
            this.CreateCustomerForm.controls["varri"].enable();
            this.CreateCustomerForm.controls["mobileTwoNo"].enable();
            this.CreateCustomerForm.controls["kmread"].enable();
            this.CreateCustomerForm.controls["email"].enable();
            this.CreateCustomerForm.controls["dob"].enable();
  ...
  // cancel the update of the customer details
  ...
  calculateDiscount(eventcheck) {
    if (eventcheck == "%") {
      ...
      this.totalBalance = Math.round(this.totalPayable - this.totalPaid);
      this.billedAmount = Math.round(this.totalPayable - this.totalPaid);
    } else {
      this.totalPayable = Math.round(
        this.totalAmount - this.advanceAmount - this.discount
      );
      //this.totalPaid=this.totalPayable
      this.totalBalance = Math.round(this.totalPayable - this.totalPaid);
      this.billedAmount = Math.round(this.totalPayable - this.totalPaid);
  ...
  // set the delivery time of the vehcile
  deliveryTime(event) {
    this.dtime = false;
  ...
  createShortInvoice(mode) {
    ...
    // this.general.sendsms().subscribe(send=>{
    //   console.log(send)
    // })
    ...
    if (mode == "update") {
      ...
      /*currentdateforcard.getFullYear().toString()+"-"+("0" + (currentdateforcard.getMonth()+1)).slice(-2).toString()+"-"+("0" + currentdateforcard.getDate()).slice(-2).toString()+" "+currentdateforcard.getHours().toString()+":"+("0" + (currentdateforcard.getMinutes())).slice(-2).toString()+":"+("0" + currentdateforcard.getSeconds()).slice(-2).toString()
       */
      ...
      .subscribe(
        (createResult) => {
          ...
          if (createResult.success == true) {
              ...
              if (this.smsAlretValue == true) {
                if (this.customerProfileMobile2 == 0) {
                  var message =
                    ...
                    " is ready. Apprx. Bill Amount: Rs." +
                    this.jobcardArrayForEdit.final_amount +
        
                    "/- Pickup your Vehicle.Thanks. " +
                    this.userService.getData()["workshop_name"] +
                    " " +
                    this.customerProfileMobile +
                    "\n(Powered by TTN)";
            ...
            this.createJobcardRequested = false;
            this.showspinner.setSpinnerForLogin(false);
            this.snakBar.open(
              "Message",
              ErrorMessgae[0][createResult["message"]],
  ...
  //remove the disable from the fields to edit the customer details
  removedisable() {
    this.shoeupdate = true;
    this.showdisbale = false;
    this.inputcolor = "#fff";
  ...
  // select the company while create lube
  ...
  //search the company while create lube
  ...
  //get the disocunt type in rupee
  ...
  getTotal() {
    ...
    // console.log(this.totaldatatable);
  }
  ...
  checkjobgst(event, data, index, gstVlaue) {
    ...
    if (data["sale_gst_rate"] != "" && data["sale_tax_type"] != "") {
      ...
      data["showcalcluationinfo"] = true;
      if (gstVlaue != "gstNull") {
        if (data["sale_tax_type"] == "Inclusive") {
      ...
      data["showcalcluationinfo"] = true;
      if (gstVlaue != "gstNull") {
        if (data["purchase_tax_type"] == "Inclusive") {
          this.jobdatatabel[index]["amount"] = amounttocalculate;
        } else {
          this.jobdatatabel[index]["amount"] = totalcal[0]["totalamount"];
        }
      }
  ...
  // calculate the final amount of jobcard
  ...
  //shre the jobcard edetails on whatsapp
  sharewhatsApp() {
  ...
  addToInventory(LubeSearchData, invenArray){
    for (var i = 0; i < LubeSearchData.lubedata.length; i++) {
      this.showspinner.setSpinnerForLogin(true);
      var vehdata = [];
      vehdata = JSON.parse(LubeSearchData.lubedata[i].vechile_details);
